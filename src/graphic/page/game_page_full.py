...
class GamePage(ParentPage):
    ...
    def update(self) -> None:
        ...
        if not self.is_paused:
            ...
            if not self.pacman.is_dead:
                ...
                gained_score = self.pacgum_manager.collect_pacgums(
                    screen_px, screen_py
                )
    ...
    def render(self) -> None:
        ...
        if self.pacman.is_dead:
            ...
            ghost_status = (
                "EDIBLE (Frightened)" if self.ghost.is_edible else "CHASE MODE"
            )
            pr.draw_text(
                f"SCORE: {self.score}   |   GHOST: {ghost_status}",
                40,
                b_y + 28,
                20,
                pr.RAYWHITE,
        ...
        if self.is_paused:
            ...
            pr.draw_rectangle_rounded(
                pr.Rectangle(box_x, box_y, box_w, box_h), 0.15, 4, pr.DARKBLUE
            )
            pr.draw_rectangle_rounded_lines(
                pr.Rectangle(box_x, box_y, box_w, box_h), 0.15, 4, pr.GOLD
            )
